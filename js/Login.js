...
$(function(){
    $("#login_button").on("click", function(){
        ...
        if(!validate_form()){
            ...
            var qry = {email:email.val(),password:password.val()};
            ...
            $.ajax({
                ...
                success: function(response){
                    ...
                    if(response.success){
                        var obj={UserInfo:{AuthID:response.data.AuthID,FullName:response.data.FullName,isLogged:true,Email:response.data.Email}};
                        chrome.storage.local.set(obj,function(){
                            if(chrome.runtime.error) {
                                NotifyUser("ERROR OCCURRED","Unable to set your Authentication. Please try again");

                            }else{
                                NotifyUser("Welcome, "+response.data.FullName,"You have successfully Logged In");
                                isLogged=true;
                                setTimeout(function(){
